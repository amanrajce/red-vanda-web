...
import { ArrowRight, Layers, Zap, Globe, Cpu, ChevronDown, Activity, BarChart3, ShieldCheck, Network, MapPin, TrendingUp } from "lucide-react";

export default function Home() {
  const spotlightRef = useRef<HTMLDivElement>(null);
  
  // Ref tracking for performance
  ...
  return (
    <div className="relative bg-white min-h-screen text-slate-900 overflow-hidden selection:bg-orange-100 selection:text-orange-900">
      ...
      <div className="relative z-20 border-y border-slate-200 bg-white/60 backdrop-blur-md py-6 overflow-hidden">
        <Container>
          <div className="flex flex-wrap justify-center md:justify-between gap-6 md:gap-12 text-xs md:text-sm font-bold uppercase tracking-widest text-slate-500">
            <span className="flex items-center gap-2"><Cpu size={16} className="text-orange-600" /> Industrial AI</span>
            <span className="hidden md:inline text-slate-300">|</span>
            <span className="flex items-center gap-2"><Zap size={16} className="text-orange-600" /> Energy Sovereignty</span>
            <span className="hidden md:inline text-slate-300">|</span>
            <span className="flex items-center gap-2"><Layers size={16} className="text-orange-600" /> Hard Tech</span>
            <span className="hidden md:inline text-slate-300">|</span>
            <span className="flex items-center gap-2"><Globe size={16} className="text-orange-600" /> Cross-Border</span>
          </div>
        </Container>
      </div>
        ...
        <Container>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 auto-rows-[minmax(300px,auto)] lg:auto-rows-[minmax(450px,auto)]">
            ...
            <div className="group md:col-span-2 lg:col-span-2 relative overflow-hidden rounded-[2rem] bg-slate-900 border border-slate-800 p-6 sm:p-8 transition-all hover:shadow-2xl hover:shadow-orange-500/20 hover:-translate-y-1 text-white flex flex-col justify-between">
              ...
              <div className="absolute inset-0 bg-[url('https://upload.wikimedia.org/wikipedia/commons/e/ec/World_map_blank_without_borders.svg')] bg-no-repeat bg-cover bg-[center_30%] opacity-20 invert mix-blend-overlay pointer-events-none"></div>
