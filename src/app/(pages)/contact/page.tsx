...
import { Upload, CheckCircle, AlertCircle, FileText, MapPin, Mail, Send, ChevronDown } from "lucide-react";
import { supabase } from "@/lib/supabase";

export default function ContactPage() {
  const [isProductCompany, setIsProductCompany] = useState<boolean | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setStatusMessage("Encrypting & Uploading Deck...");
    ...
    try {
      ...
      // --- THE FIX IS HERE ---
      setSubmitted(true);
      // Immediately scroll to the top of the page so they see the success message
  ...
  return (
    <>
      <section className="relative pt-32 pb-20 lg:pt-52 lg:pb-32 bg-slate-900 text-white overflow-hidden">
        <Container className="relative z-10">
          <div className="max-w-4xl">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/10 border border-white/20 rounded-full text-xs font-bold uppercase tracking-widest mb-8">
              <span className="w-2 h-2 rounded-full bg-orange-500 animate-pulse"></span>
              Deal Flow Open
            </div>
            
            <h1 className="text-4xl md:text-6xl lg:text-7xl font-serif font-bold leading-[1.05] mb-8">
              Pitch the <br />
              <span className="text-slate-500 italic font-light">Partnership.</span>
            </h1>
        ...
        <Container>
          <div className="grid lg:grid-cols-12 gap-16 lg:gap-24">
            ...
            <div className="lg:col-span-7">
              ...
              <div className="mb-12 p-8 bg-slate-50 border border-slate-200 rounded-2xl shadow-sm">
                <h3 className="text-lg font-serif font-bold text-slate-900 mb-6">
                  First, tell us what you are building. <span className="text-red-500">*</span>
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <button 
                    onClick={() => setIsProductCompany(true)}
                    className={`p-5 rounded-xl border-2 text-left transition-all duration-300 ${isProductCompany === true ? 'border-orange-600 bg-orange-50 shadow-md ring-1 ring-orange-200' : 'border-slate-200 hover:border-slate-300 bg-white hover:shadow-sm'}`}
                  >
                    <div className="font-bold text-slate-900 mb-1 text-lg">Product / Tech</div>
                    <div className="text-xs text-slate-500 font-medium">Scalable IP, SaaS, DeepTech</div>
                  </button>
                  
                  <button 
                    onClick={() => setIsProductCompany(false)}
                    className={`p-5 rounded-xl border-2 text-left transition-all duration-300 ${isProductCompany === false ? 'border-slate-900 bg-slate-100' : 'border-slate-200 hover:border-slate-300 bg-white hover:shadow-sm'}`}
                  >
                    <div className="font-bold text-slate-900 mb-1 text-lg">Service / Agency</div>
                    <div className="text-xs text-slate-500 font-medium">Consulting, Dev Shop, SMB</div>
                  </button>
                </div>
              </div>
              ...
              {isProductCompany === false && (
                <div className="p-8 bg-slate-100 rounded-2xl border border-slate-200 text-center animate-fade-in">
                  <AlertCircle className="w-10 h-10 text-slate-400 mx-auto mb-4" />
                  <h3 className="text-xl font-bold text-slate-900 mb-2">We might not be the right fit.</h3>
                  <p className="text-slate-600 max-w-md mx-auto">
                    Red Vanda Partners exclusively invests in scalable product and deep-tech companies. 
                    We do not invest in service agencies, consultancies, or traditional SMEs.
                  </p>
                </div>
              )}

              {/* LOGIC: SHOW PITCH FORM */}
              {isProductCompany === true && (
                <div className="animate-slide-up">
                  {submitted ? (
                    <div className="p-12 bg-green-50 border border-green-100 rounded-2xl text-center">
                      <div className="w-16 h-16 bg-green-100 text-green-600 rounded-full flex items-center justify-center mx-auto mb-6">
                        <CheckCircle className="w-8 h-8" />
                      </div>
                      <h3 className="text-2xl font-serif font-bold text-green-900 mb-2">Pitch Received.</h3>
                      <p className="text-green-800 mb-6">
                        Our investment team will review your deck. If there is alignment with our thesis, 
                        we will reach out within 7 days.
                      </p>
                      <button 
                        onClick={() => { setSubmitted(false); setIsProductCompany(null); }}
                    ...
                    <form onSubmit={handleSubmit} className="space-y-8">
                      ...
                      <div className="space-y-6">
                        <h4 className="text-xs font-bold uppercase tracking-widest text-slate-400 border-b border-slate-100 pb-2">Founder Details</h4>
                      ...
                      <div className="space-y-6">
                        <h4 className="text-xs font-bold uppercase tracking-widest text-slate-400 border-b border-slate-100 pb-2">Company Profile</h4>
                        <div className="grid md:grid-cols-2 gap-6">
                          ...
                          <div>
                            <label className="block text-sm font-medium text-slate-700 mb-2">
                              Website / LinkedIn <span className="text-red-500">*</span>
                            </label>
                            <input 
                              required 
                              name="website"
                      ...
                      {/* Section: The Pitch Deck */}
                      <div className="space-y-4">
                        <div className="flex justify-between items-center border-b border-slate-100 pb-2">
                          <h4 className="text-xs font-bold uppercase tracking-widest text-slate-400">The Pitch Deck</h4>
                          ...
                          {file ? (
                            ...
                            <div>
                              ...
                              <p className="text-sm font-medium text-slate-700 group-hover:text-slate-900">
                                Click to upload your Pitch Deck <span className="text-red-500">*</span>
                              </p>
                      ...
                      {/* Section: Message */}
                      <div className="space-y-4">
                          <h4 className="text-xs font-bold uppercase tracking-widest text-slate-400 border-b border-slate-100 pb-2">Executive Summary</h4>
                          <div>
                            <label className="block text-sm font-medium text-slate-700 mb-2">
                            ...
                            <textarea 
                              ...
                              onChange={handleChange}
                              className="w-full px-4 py-3 bg-white border border-slate-200 rounded-lg focus:ring-2 focus:ring-orange-500 outline-none transition-all placeholder:text-slate-400" 
                              placeholder="Tell us about the problem, solution, and your unique insight..." 
                            />
                          </div>
                      </div>
                      ...
                      >
                        {isSubmitting ? statusMessage : (
                          <>
                            Submit Pitch <Send className="w-4 h-4 ml-1" />
                          </>
                        )}
                      </button>
                      
                      <p className="text-xs text-center text-slate-500 mt-4">
                        By submitting, you agree that your data will be reviewed by our investment committee. 
